...
const SIDE_CALLOUTS = [
  {
    lines: ["Observe & Analyze", "In Real-Time"],
  },
  {
    lines: ["Decide & Act", "Before Stockouts"],
  },
] as const;
    ...
    <div className="relative z-20 mx-auto flex w-full max-w-[370px] flex-col items-center gap-6 px-4 desktop-sm:mx-0 desktop-sm:max-w-none desktop-sm:items-stretch desktop-sm:gap-0 desktop-sm:px-0">
      ...
      <div className="flex w-full flex-col items-center gap-3 text-center desktop-sm:max-w-[923px] desktop-sm:items-start desktop-sm:gap-6 desktop-sm:text-left">
        ...
        <div className="flex w-full flex-col items-center gap-2 desktop-sm:items-start desktop-sm:gap-0">
          ...
          >
            SmartFulfill AI transforms warehouse operations with intelligent order prioritization, inventory allocation, picking optimization, exception handling and real-time operational intelligence.
          </motion.p>
        </div>
      </div>
      ...
      >
        <Button
          ...
          className="w-full desktop-sm:w-fit font-semibold"
        >
          Open Command Center
        </Button>
        <Button
          variant="secondary"
          aria-label="Explore the Solution"
          onClick={onBookDemo}
          className="w-full desktop-sm:w-fit font-medium"
        >
      ...
      <motion.p
        ...
        className="mt-auto hidden max-w-[500px] font-tight text-[18px] leading-[25.5px] tracking-[-0.36px] text-white/80 desktop-sm:mt-[148px] desktop-sm:block text-pretty"
      >
        SmartFulfill AI transforms warehouse operations with intelligent order prioritization, inventory allocation, picking optimization, exception handling and real-time operational intelligence.
      </motion.p>

      {/* Desktop side callouts — right column */}
      <motion.aside
        initial={false}
        animate={showContent ? { opacity: 1, y: 0 } : { opacity: 0, y: 16 }}
